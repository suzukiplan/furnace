...
const char* FurnaceGUI::noteName(short note, short octave)
{
    ...
    if (settings.flatNotes) {
        if (settings.germanNotation)
            return noteNamesGF[seek];
...
void FurnaceGUI::encodeMMLStr(String& target, int* macro, int macroLen, int macroLoop, int macroRel, bool hex, bool bit30)
{
    ...
    for (int i = 0; i < macroLen; i++) {
        ...
        if (i == macroRel)
            target += "/ ";
        if (bit30 && ((macro[i] & 0xc0000000) == 0x40000000 || (macro[i] & 0xc0000000) == 0x80000000))
            target += "@";
        int macroVal = macro[i];
        if (macro[i] < 0) {
            if (!(macroVal & 0x40000000))
                macroVal |= 0x40000000;
        } else {
            if (macroVal & 0x40000000)
                macroVal &= ~0x40000000;
        ...
        switch (i) {
            ...
            case ' ':
                if (hasVal) {
                    ...
                    negaBuf = false;
                    if (macro[macroLen] < macroMin)
                        macro[macroLen] = macroMin;
                    if (macro[macroLen] > macroMax)
                        macro[macroLen] = macroMax;
                    macroLen++;
                    buf = 0;
                }
                break;
...
void FurnaceGUI::autoDetectSystemIter(std::vector<FurnaceGUISysDef>& category, bool& isMatch, std::map<DivSystem, int>& defCountMap, std::map<DivSystem, DivConfig>& defConfMap, std::map<DivSystem, int>& sysCountMap, std::map<DivSystem, DivConfig>& sysConfMap)
{
    for (FurnaceGUISysDef& j : category) {
        ...
        if (!j.subDefs.empty())
            autoDetectSystemIter(j.subDefs, isMatch, defCountMap, defConfMap, sysCountMap, sysConfMap);
        if (isMatch)
            break;
    }
}
...
void FurnaceGUI::noteInput(int num, int key, int vol)
{
    ...
    if (key == GUI_NOTE_OFF) { // note off
        ...
        if (latchEffect != -1)
            pat->data[y][4] = latchEffect;
        if (latchEffectVal != -1)
            pat->data[y][5] = latchEffectVal;
    }
    if (removeIns) {
...
void FurnaceGUI::valueInput(int num, bool direct, int target)
{
    ...
    prepareUndo(GUI_UNDO_PATTERN_EDIT);
    if (target == -1)
        target = cursor.xFine + 1;
    if (direct) {
        pat->data[y][target] = num & 0xff;
    } else {
        if (pat->data[y][target] == -1)
            pat->data[y][target] = 0;
    ...
    if (cursor.xFine == 1) { // instrument
        ...
        if (curNibble) {
            if (pat->data[y][target] > e->getMaxVolumeChan(ch))
                pat->data[y][target] = e->getMaxVolumeChan(ch);
        } else {
      ...
      e->curOrders->ord[orderCursor][curOrder]=((e->curOrders->ord[orderCursor][curOrder]<<4)|num); });
        ...
        if (orderEditMode == 2 || orderEditMode == 3) {
            if (!curNibble) {
                if (orderEditMode == 2) {
                    orderCursor++;
                    if (orderCursor >= e->getTotalChannelCount())
                        orderCursor = 0;
...
#define changeLatch(x)                        \
    if (x < 0)                                \
        x = 0;                                \
    if (!latchNibble && !settings.pushNibble) \
        x = 0;                                \
    x = (x << 4) | num;                       \
...
void FurnaceGUI::keyDown(SDL_Event& ev)
{
    ...
    if (!newFilePicker->isOpened()) {
        ...
        switch (curWindow) {
            case GUI_WINDOW_PATTERN: {
                ...
                if (mapped & (FURKMOD_ALT | FURKMOD_CTRL | FURKMOD_META | FURKMOD_SHIFT))
                    break;
    ...
    if (actionI != actionMapGlobal.cend()) {
        ...
        if (action > 0) {
            if (newFilePicker->isOpened()) {
                if (action != GUI_ACTION_OCTAVE_UP && action != GUI_ACTION_OCTAVE_DOWN)
                    return;
            }
            doAction(action);
            return;
...
void FurnaceGUI::openFileDialog(FurnaceGUIFileDialogs type)
{
    ...
    switch (type) {
        case GUI_FILE_OPEN:
            if (!dirExists(workingDirSong))
                workingDirSong = getHomeDir();
        ...
        case GUI_FILE_INS_OPEN_REPLACE:
            ...
            *prevInsData = *e->getIns(curIns);
            if (!dirExists(workingDirIns))
                workingDirIns = getHomeDir();
            hasOpened = fileDialog->openLoad(
                _("Load Instrument"),
        ...
        case GUI_FILE_WAVE_OPEN_REPLACE:
            if (!dirExists(workingDirWave))
                workingDirWave = getHomeDir();
            hasOpened = fileDialog->openLoad(
                _("Load Wavetable"),
        ...
        case GUI_FILE_SAMPLE_OPEN_REPLACE:
            if (!dirExists(workingDirSample))
                workingDirSample = getHomeDir();
            hasOpened = fileDialog->openLoad(
                _("Load Sample"),
                audioLoadFormats,
                workingDirSample,
        ...
        case GUI_FILE_EXPORT_AUDIO_ONE:
            if (!dirExists(workingDirAudioExport))
                workingDirAudioExport = getHomeDir();
            hasOpened = fileDialog->openSave(
                _("Export Audio"),
                {_("Wave file"), "*.wav"},
        ...
        case GUI_FILE_EXPORT_VGM:
            if (!dirExists(workingDirVGMExport))
                workingDirVGMExport = getHomeDir();
            hasOpened = fileDialog->openSave(
                _("Export VGM"),
                {_("VGM file"), "*.vgm"},
                workingDirVGMExport,
        ...
        case GUI_FILE_EXPORT_TEXT:
            if (!dirExists(workingDirROMExport))
                workingDirROMExport = getHomeDir();
            hasOpened = fileDialog->openSave(
                ...
                (settings.autoFillSave) ? shortName : "");
            break;
        case GUI_FILE_IMPORT_TEXT_EXPORT:
            if (!dirExists(workingDirROMExport))
                workingDirROMExport = getHomeDir();
            hasOpened = fileDialog->openLoad(
                _("Import Text Export"),
                {_("text file"), "*.txt"},
                workingDirROMExport,
                dpiScale);
            break;
        ...
        case GUI_FILE_LOAD_MAIN_FONT:
            if (!dirExists(workingDirFont))
                workingDirFont = getHomeDir();
            hasOpened = fileDialog->openLoad(
                _("Select Font"),
        ...
        case GUI_FILE_IMPORT_COLORS:
            if (!dirExists(workingDirColors))
                workingDirColors = getHomeDir();
            hasOpened = fileDialog->openLoad(
                _("Select Color File"),
        ...
        case GUI_FILE_IMPORT_KEYBINDS:
            if (!dirExists(workingDirKeybinds))
                workingDirKeybinds = getHomeDir();
            hasOpened = fileDialog->openLoad(
        ...
        case GUI_FILE_IMPORT_LAYOUT:
            if (!dirExists(workingDirLayout))
                workingDirLayout = getHomeDir();
            hasOpened = fileDialog->openLoad(
        ...
        case GUI_FILE_CMDSTREAM_OPEN:
            if (!dirExists(workingDirROM))
                workingDirROM = getHomeDir();
            hasOpened = fileDialog->openLoad(
        ...
        case GUI_FILE_TEST_OPEN:
            if (!dirExists(workingDirTest))
                workingDirTest = getHomeDir();
            hasOpened = fileDialog->openLoad(
                _("Open Test"),
...
bool FurnaceGUI::importSongFromText(const String& path)
{
    ...
    struct ParsedSystem {
        ...
        double frontRear;
        String flags;
        ParsedSystem() : id(DIV_SYSTEM_NULL),
                         volume(1.0),
                         pan(0.0),
                         frontRear(0.0),
                         flags() {}
...
void FurnaceGUI::delFirstBackup(String name)
{
    ...
    while (true) {
        ...
        if (next == NULL)
            break;
        if (strstr(next->d_name, name.c_str()) != next->d_name)
            continue;
    ...
    std::sort(listOfFiles.begin(), listOfFiles.end(), [](const String& a, const String& b) -> bool { return strcmp(a.c_str(), b.c_str()) < 0; });
...
void FurnaceGUI::processDrags(int dragX, int dragY)
{
    ...
    if (orderScrollLocked) {
        if (fabs(orderScrollRealOrigin.x - dragX) > 2.0f * dpiScale || fabs(orderScrollRealOrigin.y - dragY) > 2.0f * dpiScale)
...
void FurnaceGUI::editOptions(bool topMenu)
{
    ...
    if (ImGui::MenuItem(_("cut"), BIND_FOR(GUI_ACTION_PAT_CUT)))
        doCopy(true, true, selStart, selEnd);
    if (ImGui::MenuItem(_("copy"), BIND_FOR(GUI_ACTION_PAT_COPY)))
        doCopy(false, true, selStart, selEnd);
    if (ImGui::MenuItem(_("paste"), BIND_FOR(GUI_ACTION_PAT_PASTE)))
        doPaste();
    if (ImGui::BeginMenu(_("paste special..."))) {
        if (ImGui::MenuItem(_("paste mix"), BIND_FOR(GUI_ACTION_PAT_PASTE_MIX)))
            doPaste(GUI_PASTE_MODE_MIX_FG);
        if (ImGui::MenuItem(_("paste mix (background)"), BIND_FOR(GUI_ACTION_PAT_PASTE_MIX_BG)))
        ...
        if (ImGui::MenuItem(_("paste flood"), BIND_FOR(GUI_ACTION_PAT_PASTE_FLOOD)))
            doPaste(GUI_PASTE_MODE_FLOOD);
        if (ImGui::MenuItem(_("paste overflow"), BIND_FOR(GUI_ACTION_PAT_PASTE_OVERFLOW)))
            doPaste(GUI_PASTE_MODE_OVERFLOW);
        ImGui::EndMenu();
    }
    if (ImGui::MenuItem(_("delete"), BIND_FOR(GUI_ACTION_PAT_DELETE)))
        doDelete();
    if (topMenu) {
        if (ImGui::MenuItem(_("select all"), BIND_FOR(GUI_ACTION_PAT_SELECT_ALL)))
            doSelectAll();
    }
    ImGui::Separator();
    ...
    if (ImGui::MenuItem(_("note up"), BIND_FOR(GUI_ACTION_PAT_NOTE_UP)))
        doTranspose(1, opMaskTransposeNote);
    if (ImGui::MenuItem(_("note down"), BIND_FOR(GUI_ACTION_PAT_NOTE_DOWN)))
        doTranspose(-1, opMaskTransposeNote);
    if (ImGui::MenuItem(_("octave up"), BIND_FOR(GUI_ACTION_PAT_OCTAVE_UP)))
        doTranspose(12, opMaskTransposeNote);
    if (ImGui::MenuItem(_("octave down"), BIND_FOR(GUI_ACTION_PAT_OCTAVE_DOWN)))
        doTranspose(-12, opMaskTransposeNote);
    ImGui::Separator();
    if (ImGui::MenuItem(_("values up"), BIND_FOR(GUI_ACTION_PAT_VALUE_UP)))
        doTranspose(1, opMaskTransposeValue);
    if (ImGui::MenuItem(_("values down"), BIND_FOR(GUI_ACTION_PAT_VALUE_DOWN)))
        doTranspose(-1, opMaskTransposeValue);
    if (ImGui::MenuItem(_("values up (+16)"), BIND_FOR(GUI_ACTION_PAT_VALUE_UP_COARSE)))
        doTranspose(16, opMaskTransposeValue);
    if (ImGui::MenuItem(_("values down (-16)"), BIND_FOR(GUI_ACTION_PAT_VALUE_DOWN_COARSE)))
        doTranspose(-16, opMaskTransposeValue);
    ImGui::Separator();
    ...
    ImGui::Separator();
    if (ImGui::MenuItem(_("interpolate"), BIND_FOR(GUI_ACTION_PAT_INTERPOLATE)))
        doInterpolate();
    ...
    if (ImGui::BeginMenu(_("randomize..."))) {
        if (ImGui::InputInt(_("Minimum"), &randomizeMin, 1, 16)) {
            ...
            if (randomizeMin > randomizeMax)
                randomizeMin = randomizeMax;
        }
        if (ImGui::InputInt(_("Maximum"), &randomizeMax, 1, 16)) {
            if (randomizeMax < 0)
                randomizeMax = 0;
            if (randomizeMax < randomizeMin)
                randomizeMax = randomizeMin;
            if (randomMode) {
                if (randomizeMax > 15)
                    randomizeMax = 15;
            } else {
        ...
        if (selStart.xFine > 2 || selEnd.xFine > 2 || selStart.xCoarse != selEnd.xCoarse) {
            ...
            if (randomizeEffect) {
                if (ImGui::InputScalar(_("Effect"), ImGuiDataType_S32, &randomizeEffectVal, &_ONE, &_SIXTEEN, "%.2X", ImGuiInputTextFlags_CharsHexadecimal)) {
                    if (randomizeEffectVal < 0)
                        randomizeEffectVal = 0;
                    if (randomizeEffectVal > 255)
                        randomizeEffectVal = 255;
                }
            }
        }
    ...
    if (ImGui::MenuItem(_("invert values"), BIND_FOR(GUI_ACTION_PAT_INVERT_VALUES)))
        doInvertValues();

    ImGui::Separator();

    if (ImGui::MenuItem(_("flip selection"), BIND_FOR(GUI_ACTION_PAT_FLIP_SELECTION)))
        doFlip();
    ...
    if (ImGui::InputInt(_("collapse/expand amount##CollapseAmount"), &collapseAmount, 1, 4)) {
        if (collapseAmount < 2)
            collapseAmount = 2;
        if (collapseAmount > 256)
            collapseAmount = 256;
    }
    if (ImGui::MenuItem(_("collapse"), BIND_FOR(GUI_ACTION_PAT_COLLAPSE_ROWS)))
    ...
    if (topMenu) {
        ImGui::Separator();
        if (ImGui::MenuItem(_("collapse pattern"), BIND_FOR(GUI_ACTION_PAT_COLLAPSE_PAT)))
            doAction(GUI_ACTION_PAT_COLLAPSE_PAT);
        if (ImGui::MenuItem(_("expand pattern"), BIND_FOR(GUI_ACTION_PAT_EXPAND_PAT)))
            doAction(GUI_ACTION_PAT_EXPAND_PAT);
    }

    if (topMenu) {
        ImGui::Separator();
        if (ImGui::MenuItem(_("collapse song"), BIND_FOR(GUI_ACTION_PAT_COLLAPSE_SONG)))
            doAction(GUI_ACTION_PAT_COLLAPSE_SONG);
        if (ImGui::MenuItem(_("expand song"), BIND_FOR(GUI_ACTION_PAT_EXPAND_SONG)))
            doAction(GUI_ACTION_PAT_EXPAND_SONG);
    }
...
int FurnaceGUI::processEvent(SDL_Event* ev)
{
    ...
    if (ev->type == SDL_KEYDOWN) {
        if (!ev->key.repeat && latchTarget == 0 && !wantCaptureKeyboard && !sampleMapWaitingInput && (ev->key.keysym.mod & (~(VALID_MODS))) == 0) {
            if (settings.notePreviewBehavior == 0)
                return 1;
            switch (curWindow) {
                case GUI_WINDOW_SAMPLE_EDIT:
                case GUI_WINDOW_SAMPLE_LIST: {
...
void FurnaceGUI::pointUp(int x, int y, int button)
{
    ...
    if (orderScrollLocked) {
        ...
        if (targetOrder < 0)
            targetOrder = 0;
        if (targetOrder > e->curSubSong->ordersLen - 1)
            targetOrder = e->curSubSong->ordersLen - 1;
...
#define IMPORT_CLOSE(x)                     \
    if (x)                                  \
        pendingLayoutImportReopen.push(&x); \
    x = false;
...
bool FurnaceGUI::loop()
{
    ...
    while (!quit) {
        ...
        if (secondTimer >= 1.0f)
            secondTimer = fmod(secondTimer, 1.0f);
        ...
        if (curOrder >= 0 && curOrder < e->curSubSong->ordersLen && selStart.xCoarse >= 0 && selStart.xCoarse < e->getTotalChannelCount() && selEnd.xCoarse >= 0 && selEnd.xCoarse < e->getTotalChannelCount()) {
            for (int i = 0; i < e->curSubSong->ordersLen; i++) {
                ...
                for (int j = selStart.xCoarse; j <= selEnd.xCoarse; j++) {
                    if (e->curSubSong->orders.ord[j][i] == e->curSubSong->orders.ord[j][curOrder])
                        isPatUnique = false;
                    break;
                }
                if (!isPatUnique)
        ...
        if (!mobileUI) {
                ...
                if (settings.exportOptionsLayout == 0) {
                    ...
                    if (ImGui::MenuItem(_("import text"))) {
                        if (modified) {
                            ...
                            openFileDialog(GUI_FILE_IMPORT_TEXT_EXPORT);
                        }
                    }
                }
                ImGui::Separator();
            ...
            if (ImGui::BeginMenu(settings.capitalMenuBar ? _("Edit") : _("edit"))) {
                ImGui::Text("...");
                ImGui::Separator();
                if (ImGui::MenuItem(_("undo"), BIND_FOR(GUI_ACTION_UNDO)))
                    doUndo();
                if (ImGui::MenuItem(_("redo"), BIND_FOR(GUI_ACTION_REDO)))
                    doRedo();
                ImGui::Separator();
                editOptions(true);
                ImGui::Separator();
            ...
            if (ImGui::BeginMenu(settings.capitalMenuBar ? _("Window") : _("window"))) {
                if (ImGui::BeginMenu(_("song"))) {
                    if (ImGui::MenuItem(_("song comments"), BIND_FOR(GUI_ACTION_WINDOW_NOTES), notesOpen))
                        notesOpen = !notesOpen;
                    if (ImGui::MenuItem(_("song information"), BIND_FOR(GUI_ACTION_WINDOW_SONG_INFO), songInfoOpen))
                        songInfoOpen = !songInfoOpen;
                    if (ImGui::MenuItem(_("subsongs"), BIND_FOR(GUI_ACTION_WINDOW_SUBSONGS), subSongsOpen))
                        subSongsOpen = !subSongsOpen;
                    ImGui::Separator();
                    if (ImGui::MenuItem(_("channels"), BIND_FOR(GUI_ACTION_WINDOW_CHANNELS), channelsOpen))
                        channelsOpen = !channelsOpen;
                    if (ImGui::MenuItem(_("chip manager"), BIND_FOR(GUI_ACTION_WINDOW_SYS_MANAGER), sysManagerOpen))
                        sysManagerOpen = !sysManagerOpen;
                    if (ImGui::MenuItem(_("orders"), BIND_FOR(GUI_ACTION_WINDOW_ORDERS), ordersOpen))
                        ordersOpen = !ordersOpen;
                    if (ImGui::MenuItem(_("pattern"), BIND_FOR(GUI_ACTION_WINDOW_PATTERN), patternOpen))
                        patternOpen = !patternOpen;
                    if (ImGui::MenuItem(_("pattern manager"), BIND_FOR(GUI_ACTION_WINDOW_PAT_MANAGER), patManagerOpen))
                        patManagerOpen = !patManagerOpen;
                    if (ImGui::MenuItem(_("mixer"), BIND_FOR(GUI_ACTION_WINDOW_MIXER), mixerOpen))
                        mixerOpen = !mixerOpen;
                    if (ImGui::MenuItem(_("compatibility flags"), BIND_FOR(GUI_ACTION_WINDOW_COMPAT_FLAGS), compatFlagsOpen))
                        compatFlagsOpen = !compatFlagsOpen;
                    ImGui::EndMenu();
                }
                if (ImGui::BeginMenu(_("assets"))) {
                    if (settings.unifiedDataView) {
                        if (ImGui::MenuItem(_("assets"), BIND_FOR(GUI_ACTION_WINDOW_INS_LIST), insListOpen))
                            insListOpen = !insListOpen;
                    } else {
                        if (ImGui::MenuItem(_("instruments"), BIND_FOR(GUI_ACTION_WINDOW_INS_LIST), insListOpen))
                            insListOpen = !insListOpen;
                        if (ImGui::MenuItem(_("samples"), BIND_FOR(GUI_ACTION_WINDOW_SAMPLE_LIST), sampleListOpen))
                            sampleListOpen = !sampleListOpen;
                        if (ImGui::MenuItem(_("wavetables"), BIND_FOR(GUI_ACTION_WINDOW_WAVE_LIST), waveListOpen))
                            waveListOpen = !waveListOpen;
                    }
                    ImGui::Separator();
                    if (ImGui::MenuItem(_("instrument editor"), BIND_FOR(GUI_ACTION_WINDOW_INS_EDIT), insEditOpen))
                        insEditOpen = !insEditOpen;
                    if (ImGui::MenuItem(_("sample editor"), BIND_FOR(GUI_ACTION_WINDOW_SAMPLE_EDIT), sampleEditOpen))
                        sampleEditOpen = !sampleEditOpen;
                    if (ImGui::MenuItem(_("wavetable editor"), BIND_FOR(GUI_ACTION_WINDOW_WAVE_EDIT), waveEditOpen))
                        waveEditOpen = !waveEditOpen;
                    ImGui::EndMenu();
                }
                if (ImGui::BeginMenu(_("visualizers"))) {
                    if (ImGui::MenuItem(_("oscilloscope (master)"), BIND_FOR(GUI_ACTION_WINDOW_OSCILLOSCOPE), oscOpen))
                        oscOpen = !oscOpen;
                    if (ImGui::MenuItem(_("oscilloscope (per-channel)"), BIND_FOR(GUI_ACTION_WINDOW_CHAN_OSC), chanOscOpen))
                        chanOscOpen = !chanOscOpen;
                    if (ImGui::MenuItem(_("oscilloscope (X-Y)"), BIND_FOR(GUI_ACTION_WINDOW_XY_OSC), xyOscOpen))
                        xyOscOpen = !xyOscOpen;
                    if (ImGui::MenuItem(_("volume meter"), BIND_FOR(GUI_ACTION_WINDOW_VOL_METER), volMeterOpen))
                        volMeterOpen = !volMeterOpen;
                    ImGui::EndMenu();
                }
                if (ImGui::BeginMenu(_("tempo"))) {
                    if (ImGui::MenuItem(_("clock"), BIND_FOR(GUI_ACTION_WINDOW_CLOCK), clockOpen))
                        clockOpen = !clockOpen;
                    if (ImGui::MenuItem(_("grooves"), BIND_FOR(GUI_ACTION_WINDOW_GROOVES), groovesOpen))
                        groovesOpen = !groovesOpen;
                    if (ImGui::MenuItem(_("speed"), BIND_FOR(GUI_ACTION_WINDOW_SPEED), speedOpen))
                        speedOpen = !speedOpen;
                    ImGui::EndMenu();
                }
                if (ImGui::BeginMenu(_("debug"))) {
                    if (ImGui::MenuItem(_("log viewer"), BIND_FOR(GUI_ACTION_WINDOW_LOG), logOpen))
                        logOpen = !logOpen;
                    if (ImGui::MenuItem(_("register view"), BIND_FOR(GUI_ACTION_WINDOW_REGISTER_VIEW), regViewOpen))
                        regViewOpen = !regViewOpen;
                    if (ImGui::MenuItem(_("statistics"), BIND_FOR(GUI_ACTION_WINDOW_STATS), statsOpen))
                        statsOpen = !statsOpen;
                    if (ImGui::MenuItem(_("memory composition"), BIND_FOR(GUI_ACTION_WINDOW_MEMORY), memoryOpen))
                        memoryOpen = !memoryOpen;
                    if (ImGui::MenuItem(_("command stream player"), BIND_FOR(GUI_ACTION_WINDOW_CS_PLAYER), csPlayerOpen))
                        csPlayerOpen = !csPlayerOpen;
                    ImGui::EndMenu();
                }
                ImGui::Separator();
                if (ImGui::MenuItem(_("effect list"), BIND_FOR(GUI_ACTION_WINDOW_EFFECT_LIST), effectListOpen))
                    effectListOpen = !effectListOpen;
                if (ImGui::MenuItem(_("play/edit controls"), BIND_FOR(GUI_ACTION_WINDOW_EDIT_CONTROLS), editControlsOpen))
                    editControlsOpen = !editControlsOpen;
                if (ImGui::MenuItem(_("piano/input pad"), BIND_FOR(GUI_ACTION_WINDOW_PIANO), pianoOpen))
                    pianoOpen = !pianoOpen;
                if (spoilerOpen)
                    if (ImGui::MenuItem(_("spoiler"), NULL, spoilerOpen))
            ...
            if (ImGui::BeginMenu(settings.capitalMenuBar ? _("Help") : _("help"))) {
                if (ImGui::MenuItem(_("effect list"), BIND_FOR(GUI_ACTION_WINDOW_EFFECT_LIST), effectListOpen))
                    effectListOpen = !effectListOpen;
                if (ImGui::MenuItem(_("debug menu"), BIND_FOR(GUI_ACTION_WINDOW_DEBUG)))
                    debugOpen = !debugOpen;
                if (ImGui::MenuItem(_("inspector")))
                    inspectorOpen = !inspectorOpen;
                if (ImGui::MenuItem(_("panic"), BIND_FOR(GUI_ACTION_PANIC)))
                    e->syncReset();
            ...
            if (e->isPlaying() && settings.playbackTime) {
                ...
                if (hasInfo && (settings.statusDisplay == 0 || settings.statusDisplay == 2)) {
                    ...
                    if (curFileName != "")
                        ImGui::Text("| %s", curFileName.c_str());
                }
            }
            ImGui::PopStyleColor();
            if (modified) {
        ...
        if (inspectorOpen)
            ImGui::ShowMetricsWindow(&inspectorOpen);
                    ...
                    *e->song.wave[curWave]=*wave;
                    MARK_MODIFIED; });
                        ...
                        case GUI_FILE_IMPORT_CONFIG:
                            ...
                            break;
                        case GUI_FILE_IMPORT_TEXT_EXPORT:
                            if (!importSongFromText(copyOfName)) {
                                showError(fmt::sprintf(_("Error while importing text export! (%s)"), lastError));
                            }
                            break;
                        case GUI_FILE_EXPORT_COLORS:
                ...
                e->lockEngine(
                    [this, progressLambda, curPosInRowsLambda, curFileLambda, loopsLeftLambda, totalLoopsLambda]() {
                        ...
                        *curPosInRowsLambda = curRow;
                        for (int i = 0; i < MIN(curOrder, (int)songOrdersLengths.size()); i++)
        ...
        if (ImGui::BeginPopupModal(_("ROM Export Progress"), NULL)) {
            if (pendingExport == NULL) {
                ...
                if (romLogSize.y < 60.0f * dpiScale)
                    romLogSize.y = 60.0f * dpiScale;
            ...
            switch (warnAction) {
                ...
                case GUI_WARN_CLEAR:
                    if (ImGui::BeginTable("EraseOpt", 2, ImGuiTableFlags_BordersInnerV)) {
                        ...
                        if (ImGui::Button(_("Current subsong"))) {
                            stop();
                            e->lockEngine([this]() { e->curSubSong->clearData(); });
                            e->setOrder(0);
                            curOrder = 0;
                            cursor.order = 0;
                            selStart.order = 0;
                  ...
                  for (int j=0; j<DIV_MAX_ROWS; j++) {
                    ...
                    pat->data[j][1]=0;
                        ...
                        if (ImGui::Button(_("Instruments"))) {
                            stop();
                            e->lockEngine([this]() { e->song.clearInstruments(); });
                            curIns = -1;
                            MARK_MODIFIED;
                            ImGui::CloseCurrentPopup();
                        }
                        if (ImGui::Button(_("Wavetables"))) {
                            stop();
                            e->lockEngine([this]() { e->song.clearWavetables(); });
                            curWave = 0;
                            MARK_MODIFIED;
                            ImGui::CloseCurrentPopup();
                        }
                        if (ImGui::Button(_("Samples"))) {
                            stop();
                            e->lockEngine([this]() { e->song.clearSamples(); });
                            curSample = 0;
                            MARK_MODIFIED;
                            ImGui::CloseCurrentPopup();
                        }
                ...
                e->curSubSong->rearrangePatterns(); });
                        ...
                        if (ImGui::Button(_("Remove unused patterns"))) {
                            stop();
                            e->lockEngine([this]() { e->curSubSong->removeUnusedPatterns(); });
                            MARK_MODIFIED;
                            ImGui::CloseCurrentPopup();
                        }
                ...
                case GUI_WARN_IMPORT_TEXT:
                    if (ImGui::Button(_("Yes"))) {
                        ...
                        if (curFileName == "" || curFileName.find(backupPath) == 0 || e->song.version >= 0xff00) {
                            openFileDialog(GUI_FILE_SAVE);
                            postWarnAction = GUI_WARN_IMPORT_TEXT;
                        } else {
                            if (save(curFileName, e->song.isDMF ? e->song.version : 0) > 0) {
                                showError(fmt::sprintf(_("Error while saving file! (%s)"), lastError));
                            } else {
            ...
            if (displayInsTypeListMakeInsSample == -2) {
                ...
                if (!makeDrumkitMode) {
                    ...
                    if (ImGui::InputInt("##DKOctave", &makeDrumkitOctave, 1, 3)) {
                        if (makeDrumkitOctave < 0)
                            makeDrumkitOctave = 0;
                        if (makeDrumkitOctave > 9)
                            makeDrumkitOctave = 9;
                    }
                }
                ...
                if (ImGui::MenuItem(temp)) {
                    ...
                    if (curIns == -1) {
                        ...
                        if (displayInsTypeListMakeInsSample == -2) {
                            ...
                            if (i != DIV_INS_AMIGA)
                                e->song.ins[curIns]->amiga.useSample = true;
            ...
            if (ImGui::BeginTable("PendingSamplesList", 1, ImGuiTableFlags_ScrollY, ImVec2(0.0f, sizeY))) {
                ...
                } else // display search results
                {
                    ...
                    for (size_t i = 0; i < sampleBankSearchResults.size(); i++) {
                        ...
                        if (sampleBankSearchResults[i].second)
                            anySelected = true;
                    }
        ...
        if (ImGui::BeginPopupModal(_("Import Raw Sample"), NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
            ...
            for (int i = 0; i < DIV_SAMPLE_DEPTH_MAX; i++) {
                if (sampleDepths[i] == NULL)
                    continue;
                if (ImGui::RadioButton(sampleDepths[i], pendingRawSampleDepth == i))
                    pendingRawSampleDepth = i;
            }
            ...
            if (ImGui::InputInt("##RSRate", &pendingRawSampleRate, 100, 1000)) {
                if (pendingRawSampleRate < 100)
                    pendingRawSampleRate = 100;
                if (pendingRawSampleRate > 384000)
                    pendingRawSampleRate = 384000;
            }
...
bool FurnaceGUI::init()
{
    ...
    if (backupPath.size() > 0) {
        if (backupPath[backupPath.size() - 1] == DIR_SEPARATOR)
            backupPath.resize(backupPath.size() - 1);
    }
    ...
    for (int i = 0; i < value; i++) {
        SF_FORMAT_INFO f;
        f.format = i;
        if (sf_command(NULL, SFC_GET_FORMAT_MAJOR, &f, sizeof(SF_FORMAT_INFO)) != 0)
            continue;
        logV("format %d: %s (%s)\n", i, f.name, f.extension);
        // these two are/will be handled somewhere else
        if (strcmp(f.extension, "raw") == 0)
            continue;
        if (strcmp(f.extension, "xi") == 0)
            continue;
        // just in case
        if (strcmp(f.extension, "dmc") == 0)
            continue;
        if (strcmp(f.extension, "brr") == 0)
            continue;
...
void FurnaceGUI::syncState()
{
    ...
    if (audioExportOptions.loops < 0)
        audioExportOptions.loops = 0;
    audioExportOptions.fadeOut = e->getConfDouble("exportFadeOut", 0.0);
    if (audioExportOptions.fadeOut < 0.0)
        audioExportOptions.fadeOut = 0.0;
