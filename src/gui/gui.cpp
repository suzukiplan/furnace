...
const char *FurnaceGUI::noteName(short note, short octave)
{
  ...
  { // note off and envelope release
    return noteRelLabel;
  }
  else if (note == 102)
  { // envelope release only
...
void FurnaceGUI::noteInput(int num, int key, int vol)
{
  ...
  else if (key == GUI_NOTE_OFF_RELEASE)
  { // note off + env release
    pat->data[y][0] = 101;
    pat->data[y][1] = 0;
    removeIns = true;
  }
  else if (key == GUI_NOTE_RELEASE)
  { // env release only
    pat->data[y][0] = 102;
    pat->data[y][1] = 0;
    removeIns = true;
        ...
        if (fileName != "")
        {
          ...
          case GUI_FILE_INS_OPEN_REPLACE:
          {
            ...
            if (!instruments.empty())
            {
              ...
              { // replace with the only instrument
                if (curIns >= 0 && curIns < (int)e->song.ins.size())
                {
            ...
            [this, progressLambda, curPosInRowsLambda, curFileLambda, loopsLeftLambda, totalLoopsLambda]()
            {
              ...
              *curPosInRowsLambda = curRow;
              ...
              if (!songHasSongEndCommand)
              {
                ...
                if ((*totalLoopsLambda) != (*loopsLeftLambda))
                {                                                                // we are going 2nd, 3rd, etc. time through the song
                  *curPosInRowsLambda -= (songLength - songLoopedSectionLength); // a hack so progress bar does not jump?
                }
                if (e->getIsFadingOut())
                { // we are in fadeout??? why it works like that bruh
